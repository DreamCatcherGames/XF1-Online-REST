...
namespace XF1_Online_REST.LogicScripts
{
    public class Racing_TeamLogic
    {
        ...
        public HttpResponseMessage searchRacingTeams(string token, string salt, int amount, int page, string name, string country)
        {
            Error_List errors = new Error_List();
            errors.addError("Invalid token", tools.verifyToken(token, salt, "Administrator") || tools.verifyToken(token, salt, "Player"));
            if (!errors.hasErrors())
            {
                ...
                var pilotsPage = dbContext.SP_filterRacingTeams(name,country).OrderByDescending(o => o.Price).Skip(page * amount).Take(amount).ToList();
